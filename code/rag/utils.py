...
def _build_client_kwargs(api_key=None, base_url=None):
...
def count_tokens(text, model=None):
    ...
    try:
        enc = tiktoken.encoding_for_model(resolved_model)
    except KeyError:
...
def ChatGPT_API_with_finish_reason(model, prompt, api_key=None, chat_history=None):
    max_retries = 10
    client = openai.OpenAI(**_build_client_kwargs(api_key=api_key))
    resolved_model = _resolve_model(model)
...
async def ChatGPT_API_async(model, prompt, api_key=None):
    ...
    for i in range(max_retries):
        try:
            async with openai.AsyncOpenAI(**_build_client_kwargs(api_key=api_key)) as client:
...
def get_page_tokens(pdf_path, model=DEFAULT_CHAT_MODEL, pdf_parser="pypdf"):
    resolved_model = _resolve_model(model)
    try:
        enc = tiktoken.encoding_for_model(resolved_model)
    except KeyError:
    ...
    parser_name = str(pdf_parser).lower()
    if parser_name in {"pypdf", "pypdf2"}:
        pdf_reader = PdfReader(pdf_path)
        page_list = []
        for page_num in range(len(pdf_reader.pages)):
            page = pdf_reader.pages[page_num]
        ...
        if pymupdf is None:
            raise ImportError("PyMuPDF 未安装，无法使用 pymupdf 解析 PDF")
...
def post_processing(structure, end_physical_index):
    ...
    for i, item in enumerate(structure):
        ...
        if (
            item.get('start_index') is not None
            and item.get('end_index') is not None
